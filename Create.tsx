...
import { saveDraft, loadDraft, clearDraft, type Draft } from "@/lib/draft";
...
export default function Create() {
  ...
  /* Restore a saved draft once per visit — so signing in or creating an
     account mid-edit brings the customer back to exactly where they were. */
  useEffect(() => {
    ...
    loadDraft(key).then(d => {
      if (!d) return;
      setOrient(d.orient);
      setAthlete(d.athlete); setLine2(d.line2);
      setCityName(d.cityName); setNotes(d.notes);
      setColorMode(d.colorMode); setInkArt(d.inkArt); setInkText(d.inkText);
      setSchool(d.school); setSport(d.sport);
      if (d.photo) {
        onFile(new File([d.photo.blob], d.photo.name, { type: d.photo.type }));
      }
      if (d.logo) {
        const f = new File([d.logo.blob], d.logo.name, { type: d.logo.type });
        logoRef.current = f;
        setLogoUrl(URL.createObjectURL(f)); setLogoName(f.name);
      }
      if (key === "city" && d.map) {
        setFly({ lat: d.map.lat, lng: d.map.lng, zoom: d.map.zoom });
      }
      setRestored(true);
    });
  }, [key]);

  /* Quietly save the work-in-progress as they edit (debounced). */
  useEffect(() => {
    const dirty = hasPhoto || !!logoUrl || !!athlete || !!line2 ||
      !!cityName.trim() || !!notes || !!school || !!sport;
    if (!dirty) return;
    if (saveT.current) window.clearTimeout(saveT.current);
    saveT.current = window.setTimeout(() => {
      const mk = (f: File | null) =>
        f ? { blob: f as Blob, name: f.name, type: f.type } : null;
      let map: Draft["map"] = null;
      if (key === "city" && frameRef.current) {
        try {
          const fr = frameRef.current();
          map = { lat: fr.center[0], lng: fr.center[1], zoom: fr.zoom };
        } catch { map = null; }
      }
      saveDraft(key, {
        savedAt: Date.now(), orient, athlete, line2, cityName, notes,
        colorMode, inkArt, inkText, school, sport,
        photo: mk(fileRef.current), logo: mk(logoRef.current), map,
      });
    }, 600);
    return () => { if (saveT.current) window.clearTimeout(saveT.current); };
  }, [key, orient, athlete, line2, cityName, notes, colorMode, inkArt,
      inkText, school, sport, hasPhoto, logoUrl]);

  function startFresh() {
    clearDraft(key);
    fileRef.current = null; logoRef.current = null;
    setHasPhoto(false); setPhotoUrl(""); setVerdict(null); setLowRes(null);
    setLogoUrl(""); setLogoName(""); setAthlete(""); setLine2("");
    setCityName(""); setNotes(""); setSchool(null); setSport("");
    setColorMode("single"); setInkArt("#082b4a"); setInkText("#082b4a");
    setOrient(PRODUCTS[key].defaultOrientation);
    setRestored(false);
  }
      ...
      <div className="bg-paper rounded-lg shadow-sheet border border-ink/10 p-7 h-fit">
        {restored && (
          <div className="mb-4 rounded-md border border-ink/15 bg-ink/5 px-4 py-3
            ...
            <button type="button"
                    className="caption underline whitespace-nowrap"
                    onClick={startFresh}>
              Start fresh
            </button>
          </div>
        ...
        {session ? (
          ...
          <>
            ...
            <p className="caption mt-2 text-center">
              Your work here is saved on this device &mdash; signing in or
              creating an account won&rsquo;t lose it.
            </p>
          </>
        )}
        {session && !ready && (
